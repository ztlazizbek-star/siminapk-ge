...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $code = $input['code'] ?? '';
    $type = $input['type'] ?? 'verification'; // 'verification' or 'notification'
    ...
    if ($type === 'verification' && empty($code)) {
        echo json_encode(['success' => false, 'error' => 'Код обязателен для верификации']);
        exit;
    }
    
    if ($type === 'notification' && empty($message_text)) {
        echo json_encode(['success' => false, 'error' => 'Сообщение обязательно для уведомления']);
        exit;
    }
    ...
    if ($DEV_MODE) {
        $dev_message = $type === 'verification' ? $code : $message_text;
        error_log("[SERVER][v0] DEV MODE: SMS would be sent to $phone_number with message: $dev_message");
        echo json_encode([
            'success' => true,
            'dev_mode' => true,
            'msg_id' => 'dev_' . uniqid(),
            ...
            'phone' => $phone_number,
            'type' => $type,
            'message' => 'DEV MODE: SMS не отправлено, используйте из консоли'
        ]);
        exit;
    }
    ...
    if ($type === 'verification') {
        ...
        // For notifications, send the message as-is without prefix
